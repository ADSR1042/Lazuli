...
$(document).ready(async function () {
	...
	//如果是查老师的根目录 即url为 chalaoshi.click 或者 http://chalaoshi-click-s.webvpn.zju.edu.cn:8001/ url需要完全匹配
	if (window.location.href == 'https://chalaoshi.click/' || window.location.href == 'http://chalaoshi-click-s.webvpn.zju.edu.cn:8001/') {
		//获取当前时间
	...
	//如果是zdbl选课页面 url包含 http://zdbk.zju.edu.cn/jwglxt/xsxk
		...
		if (!localTime || new Date().getTime() - localTime > expireTime) {
			...
			if (globalConfig.enableDataExpirationReminders) {
				desktop_notification('选课插件提示', '评分数据已过期，点击打开查老师页面更新评分', 20000, 'http://chalaoshi.click/');
				//此处暂时不返回 避免影响后续代码执行
				...
				teacherNames.forEach((teacherName) => {
					...
					if (res && res.rate) {
						...
						if (parseFloat(res.rate) > 8.5) {
							scoreHTML += '<a style="color:red;" href=https://chalaoshi.click/t/' + res.id + ' target="_blank" >' + res.rate + '</a><br>';
						}
						//如果评分小于2 设置为紫色
						else if (parseFloat(res.rate) < 2) {
							scoreHTML += '<a style="color:#4340ff;" href=https://chalaoshi.click/t/' + res.id + ' target="_blank" >' + res.rate + '</a><br>';
						}
						// 正常情况黑色
						else {
							scoreHTML += '<a style="color:black;" href=https://chalaoshi.click/t/' + res.id + ' target="_blank" >' + res.rate + '</a><br>';
						}
